const physics = {
  ...
  levelFinish(data) {
    ...
    // Beri jeda animasi sebentar, lalu munculkan pop-up menang (ditangani app.js)
    setTimeout(() => {
      data.paused = true;
      if (typeof window !== 'undefined' && window.dispatchEvent) {
        window.dispatchEvent(new CustomEvent('mario-win'));
      }
    }, 2500);
  },
  ...
  sceneryCollision(data, entity, scene) {
    ...
    if (entity.yPos >= scene.yPos &&
      ...
      entity.xPos < (scene.xPos + scene.width) && entity.velY < 0) {
      if (scene.type === 'block') {
        if (scene.contents === 'quiz') {
          if (!scene.quizDone && entity.type === 'mario') {
            scene.quizDone = true;
            scene.sprite = scene.used;
            data.paused = true; // freeze game sampai quiz dijawab
            if (typeof window !== 'undefined' && window.dispatchEvent) {
              window.dispatchEvent(
                new CustomEvent('mario-quiz', { detail: { quizId: scene.quizId } }));
            }
          }
        } else if (scene.contents === 'coin') {
